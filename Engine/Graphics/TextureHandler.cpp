...
void TextureHandler::CreateTexture(const std::string& textureName_, const std::string& textureFilePath_)
{
	...
	Log::Info(textureFilePath_.c_str(), "TextureHandler.cpp", __LINE__);
	surface = IMG_Load(textureFilePath_.c_str());
	if (surface == nullptr)
	{
		Log::Error("Surface for texture " + textureName_ + " failed to be created", "TextureHandler.cpp", __LINE__);
		delete t;
		t = nullptr;
		return;
	}
